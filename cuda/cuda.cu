...
int main() {
    ...
    h_b = new float[N];
    h_c = new float[N];
    std::vector<float> cpu_c_vector(N);
    ...
    std::cout << "Vector Addition CPU time: " << cpu_time << " sec\n";
    std::cout << "Vector Addition GPU time: " << gpu_time / 1000.0 << " sec\n";
    std::cout << "Speedup: " << cpu_time / (gpu_time / 1000.0) << "\n";
    ...
    std::cout << "Vector Addition Results: " << (vector_match ? "Not Matched" : "Matched") << "\n\n";

    float *h_A, *h_B, *h_C, *d_A, *d_B, *d_C;
    h_A = new float[M * M];
    h_B = new float[M * M];
    h_C = new float[M * M];
    std::vector<float> cpu_C_vector(M * M);
    ...
    cpu_time = std::chrono::duration<double>(end_cpu - start_cpu).count();
    ...
    di0m3 numBlocks((M + threadsPerBlock.x - 1) / threadsPerBlock.x,
    ...
    std::cout << "Matrix Multiplication CPU time: " << cpu_time << " sec\n";
    std::cout << "Matrix Multiplication GPU time: " << gpu_time / 1000.0 << " sec\n";
    std::cout << "Speedup: " << cpu_time / (gpu_time / 1000.0) << "\n";
    ...
    std::cout << "Matrix Multiplication Results: " << (matrix_match ? "Not Matched" : "Matched") << "\n";
